...
function ImageButtonEdit_ClientClick() {
    var grid = jQuery("#" + GetClientId("jqGridDebtors") + "_datagrid1");
...
function ImageButtonReports_ClientClick() {
    var bodycorpid = querySt("bodycorpid");
    if (bodycorpid != null) {
        if (bodycorpid != "") {
            window.open('debtorreports.aspx?bodycorpid=' + bodycorpid, '_blank');
        }
        else
            window.open('debtorreports.aspx', '_blank');
    }
    else
        window.open('debtorreports.aspx', '_blank');
}